...
$demoDirectory = Join-Path $PSScriptRoot "local-demo"
...
try {
    .\gradlew.bat :standalone-server:installDist :minecraft-neoforge:build
} finally {
    Pop-Location
}

if ($PrepareOnly) {
    Write-Host "MineVOICE local demo configuration prepared."
    exit 0
}
...
Start-DemoWindow "MineVOICE Voice Server" ".\gradlew.bat :standalone-server:run --args='../scripts/local-demo/minevoice-standalone.properties'"
...
Write-Host "MineVOICE local demo started."
Write-Host "In both clients, join Multiplayer -> Direct Connection -> 127.0.0.1:$MinecraftPort"
Write-Host "Press O in either client to open MineVOICE, then test devices, group chat, and voice routing."
